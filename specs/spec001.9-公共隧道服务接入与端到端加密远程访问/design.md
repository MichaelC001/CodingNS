...
**待补：`hello` 里的协议版本没有校验**（2026-09-16 记）。
帧头里的二进制版本号是校验了的（对不上直接报「线协议版本不支持」），
但 `hello` 帧里那个 `protocolVersion` 目前只被携带、没人读。

后果是版本错配时的报错不好懂：一台新 Host 配一个旧客户端，
用户上传大文件会在**客户端**本地抛出「body 超过单帧上限」——
他不知道这是版本旧了，只会以为功能坏了。
补上校验后 Host 可以明确回一句「客户端协议版本过旧，请升级」，支持成本会低很多。
优先级不高，但不能忘。
...
## 14. 远程入口的两层认证边界（2026-09-18）

远程入口必须把两种账号分开处理，原因很简单：它们控制的事情不同。

### 14.1 CodingNS Connect 账号：证明“你有权连这台设备”

用户打开四级域名入口后，第一步只向 CodingNS Connect 控制站提交 Connect 邮箱和密码。控制站验证登录态、绑定关系并签发信令票据，客户端再用票据建立 WebRTC DataChannel。

没有有效的 Connect 登录态，客户端不能建立面向这台 Host 的隧道，也不能读取 Host 账号列表。这样可以修复“直接打开页面后请求 Host 登录，却收到需要先登录 CodingNS 账号”的错误路径。

### 14.2 CodingNS Host 账号：决定“登录后能做什么”

隧道建立后，客户端通过隧道请求 Host 的 `GET /api/public/host-login-accounts`。Host 只返回活动账号的最小信息：`userId`、`username` 和 `role`。接口必须同时检查网关注入的 `x-codingns-relay-session-id`，没有已建立的隧道就返回 403。

客户端展示账号列表只是为了让用户选择本机账号，不代表已经登录 Host。用户仍然必须输入所选 Host 账号的密码，最后继续调用原有 `POST /api/auth/login` 完成 Host 认证和权限校验。

### 14.3 明确禁止返回的内容

账号列表响应不能包含密码、密码哈希、访问 Token、刷新 Token、设备信息、会话信息或其他本地权限数据。Connect 认证成功只证明设备绑定所有权，不替代 Host 本地权限认证。
...
初始化远程入口配置
    -> CodingNS Connect 登录
    -> 建立 WebRTC 隧道并读取活动 Host 账号
    -> 选择 Host 账号并输入 Host 密码
    -> 调用原有 Host 登录接口
    -> 进入工作台
```

任何一步失败都停留在当前阶段并给出对应错误，不自动把 Connect 凭据当成 Host 凭据重试。

## 15. 登录入口的双方式收口（2026-09-19）

对应需求 14。这一节解决的是一个已经存在的边界问题，不是加新功能。

### 15.1 要修的问题

桌面端和移动端在"服务器地址"里填了四级域名后，会进入普通登录页：页面上只有 Host 账号密码，看不到 Connect 这一步。更麻烦的是，客户端登录时会先探测 Host 上报的直连候选地址（本机 127.0.0.1、局域网 IP），探到就绕开隧道直接把账号密码发给那个地址。

结果就是：只要拿到四级域名，就能尝试直连 Host 登录，既绕过了 Connect 认证，也可能被用来做恶意扫描或流量调用。

### 15.2 登录页的统一结构

登录页改成两个标签页，桌面端、移动端、H5 远程入口都用这一套：

```text
[ 直接登录 ]  [ CodingNS Connect 登录 ]
```

**直接登录**

- 只连能直连到的 Host：App 端可以用检测到的本机/局域网服务或手填直连地址；H5 端只探测本机 `127.0.0.1`（https 页面直连局域网 http 地址会被浏览器拦掉，只有本机地址被浏览器豁免）。
- 目标地址是四级域名时，不允许在这个标签页提交 Host 登录：前端直接拦住并提示改用 Connect 登录。
- 登录请求只发往直连地址，不经过中继，也不需要 Connect 账号。

**CodingNS Connect 登录**

- 第一步照旧只提交 Connect 邮箱和密码。
- Connect 认证通过后，才允许建立到目标四级域名的隧道；没有认证就不建隧道、不发 Host 请求。
- 隧道建立后按第 14 节的两层认证流程继续：读 Host 账号列表 -> 选账号输密码 -> 调用 Host 登录接口。
- PC / 移动端没有预设目标时，登录成功后先列出这个账号下的设备，选一台再继续，用户不需要自己记四级域名。

### 15.3 什么时候才显示两种登录方式

分两种情况：

- **PC / 移动端**：始终显示两个标签页。用户不需要知道远程域名，登录 Connect 后从设备列表选设备。
- **Web（H5）**：页面靠用户手输地址打开，所以只在当前连接目标本身就是四级域名入口（`*.channel.<domain>`）时才显示两个标签页。

Web 的判据只有一条：**当前连接目标地址本身就是四级域名入口**。

- 通过 `*.channel.<domain>` 打开 H5 入口页：入口页固定显示两个标签页。
- 服务器设置里把地址填成 `*.channel.<domain>`：登录页显示两个标签页，默认停在 Connect。
- 其它情况（本机地址、局域网地址、自建反代地址等）：只显示直接登录，不出现标签页，也不出现 Connect 选项。

**这里有个坑**：`HostProfile.relayTunnel` 只表示"这台 Host 在 Connect 侧有绑定"，登录成功后会被客户端运行时配置同步写进当前 Host 的 profile。它**不代表**当前连接方式走了 Connect。把 relayTunnel 有没有值当成判据，会让直连本机 Host 的用户被迫看到 Connect 选项——这一条已经在实现里修正，只看 `baseUrl`。

默认选中规则：

- 当前连接目标是四级域名：默认停在 Connect 标签页，直接登录标签页只做说明。
- 移动端（iOS / Android）且没有远程目标：默认停在 Connect（手机上的默认地址 `127.0.0.1:3002` 没有意义）。
- 其它直连目标：默认停在直接登录。
- 用户手动切过标签页之后，以用户的选择为准，不因为后台状态刷新跳回去。

### 15.4 明确禁止的路径

- 登录阶段不再对四级域名目标做直连候选回退（原来的 `login-direct-candidate-resolver` 行为删除）。直连候选只在登录成功之后的业务链路里使用（见 spec001.9.3）。
- 直接登录标签页不接受四级域名作为目标地址，即使它长得像普通主机名。
- Connect 认证没通过时，不建立隧道、不读 Host 账号列表、不提交 Host 登录。

### 15.5 与既有能力的关系

- 设置页里登录 Connect、绑定设备的能力不变，登录页只是把"连接这台设备"需要的前置步骤搬到了登录流程里。
- 直连 Host 的日常登录（桌面端连本机服务）体验不变，只是换到了「直接登录」标签页里。
- Host 侧接口不变：仍然只有 Connect 认证通过、隧道建立后，才能读账号列表；直接登录走的是 Host 原本的 `/api/auth/login`。
