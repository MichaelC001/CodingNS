...
1. 客户端向控制面请求连接信息，拿到 Host 的 DTLS 指纹、信令地址、ICE 服务器配置和信令凭据
2. 客户端连信令服务器，加入对应 Host 的会话
3. 双方交换 SDP / ICE 候选
4. 建立 DataChannel，**校验对端 DTLS 指纹**
5. 指纹一致后，客户端把业务请求封成二进制消息发出
6. Host 侧解密后转发到本地 `http://127.0.0.1:<host-port>`
7. Host 把响应回传
