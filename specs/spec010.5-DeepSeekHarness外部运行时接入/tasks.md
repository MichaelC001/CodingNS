...
- 2026-08-21 本机运行时升级：安装 `@deepseek-ai/dsh@0.1.1-rc.2`，`dsh --version` 返回 `0.1.1-rc.2`；sidecar 在 `127.0.0.1:58067` 完成 `host.describe`，Host 版本校验同步支持当前版本和 `0.1.0-rc.5` 回滚版本。未发送模型 prompt，真实模型回答仍依赖 Harness 自身凭据。
- 2026-09-14 孤儿 sidecar 回收（当前工作区，尚未单独提交）：sidecar 以 `detached` 启动，Host 被强杀或崩溃后它会被 init/launchd 收养（PPID 变成 1）继续活着，并一直占着 DSH 的会话写入租约；DSH 的租约不做超时抢占，所以新 Host resume 这些会话时会报 `SessionAlreadyOwnedError`。本机实测累积了 33 个孤儿 sidecar，占住 13 个会话锁，其中「会话部署」这条会话的锁被一个已经没有页面连接的旧 Host 持有，导致当前 GUI 无法继续发消息。现在 `startOwnedSidecar` 在启动自己的 sidecar 之前做一次性回收：只认 `dsh web --host <127.0.0.1|0.0.0.0> --port <n> --no-open` 且父进程已经是 1 的进程，按进程组回收（连带 sidecar 自己拉起的子进程），回收失败只记日志、不阻塞启动；未匹配形态的 `dsh` 进程不动，避免误杀手动启动的运行时。已验证：`pnpm --dir apps/host test -- --run tests/integration/deepseek-harness-sidecar-manager.test.ts tests/integration/deepseek-harness-sidecar-reclaim.test.ts` 通过，15/15；`pnpm exec tsc -p tsconfig.json --noEmit`（在 `apps/host`）通过。本机手工清理后只剩当前 GUI 的 sidecar，被占用的会话锁从 13 个降到 2 个。
- 2026-09-14 sidecar 不再产生孤儿：上一轮的一次性回收只是"用到的时候顺手清一次"，挡不住新孤儿长出来——它只在 `startOwnedSidecar` 之前触发，而且每个 Host 生命周期只跑一次；本轮复查时又实测到一个新的孤儿：dev Host 被 tsx watch 重启后，它 15:01 拉起的 `dsh web` 一直活着（PPID=1，占着端口 49767）。这次从两头收口。第一，新增 `scripts/dsh-sidecar-guard.cjs`，由 sidecar manager 通过 `NODE_OPTIONS=--require` 注入到 `dsh web` 进程里，同时把发起进程号用 `CODINGNS_SIDECAR_GUARD_PARENT_PID` 传进去；sidecar 每 2 秒自查一次，发现发起进程消失、或自己已经被 launchd 收养，就先给自己发 SIGTERM 走 dsh 自己的收尾流程，3 秒不响应再强退。Host 即使被 SIGKILL，sidecar 也能自己让出端口和会话租约。没有注入父进程号时守卫根本不装，用户手动跑的 `dsh` 不受影响。第二，`startHost` 在监听就绪后主动回收一次孤儿，不再等到第一次 Harness 请求才清；回收入口由 `createServer` 暴露但不在构造阶段调用，避免每个测试用例都去扫一遍本机进程表。守卫脚本沿用 `claude-hook-bridge.cjs` 的分发方式，`packages/codingns/scripts/build.mjs` 会把它一起复制进包内 `scripts/`。已验证：守卫行为测试 2/2（真起进程：发起进程被 SIGKILL 后 sidecar 2.3 秒内自退；未注入父进程号时 3 秒后仍存活）、sidecar manager 测试 11/11、reclaim 测试 6/6；`pnpm exec tsc -p tsconfig.json --noEmit`（在 `apps/host`）通过；另外用真实 `dsh` 配临时 `DSH_HOME` 做了端到端复现，假 Host 被强杀后 7 秒内 sidecar 自行退出，没有留下孤儿和占用端口。
- 2026-09-14 Host 重启改为接管已有 sidecar：上一轮解决了"孤儿不清"，但没解决"每次重启都新建"——Host 每次启动仍然是"回收旧的 + 拉起新的"，开发时 tsx watch 一天重启几十次，进程数就跟着重启次数涨。现在新增 `deepseek-harness-sidecar-registry.ts` 管一份租约：`<Host 数据目录>/deepseek-harness-sidecar.json` 记着当前 sidecar 的 pid、端口、`baseUrl`、一次性认证 URL 和"谁在用它"的进程号名单。新 Host 起来先读租约，pid 还活着就用这个认证 URL 重新换一次 cookie 并探测模型目录，通了就直接接管，一个进程都不新建。认证能跨 Host 复用是因为 DSH 的一次性 URL 只校验进程内的 token、没有消费标记，而 cookie 是用 `~/.dsh` 里持久密钥签发的无状态令牌，所以下一个 Host 随时能换到属于自己的 cookie。没有可接管的才自己拉起，拉起前后都用目录锁（`mkdir` 的原子性）串行化，两个 Host 同时启动时只有一个真的去拉进程，另一个等它写好租约后接管。守卫也跟着升级：不再只看"发起进程还在不在"，而是看租约名单里还有没有活着的 Host，名单空了才开始 60 秒接管倒计时，倒计时内没人接手才自行退出——这样 Host 正常重启能无缝交接，真崩溃也不会长期占着会话租约。租约按 Host 数据目录隔离，开发 Host 和安装版 Host 各管各的 sidecar，互不接管。已验证：租约读写/损坏容错/启动锁互斥/残留锁清理/接管不新建进程/关闭时只注销不 kill/守卫按名单收尾共 10 项新测试全过，sidecar 相关测试合计 29/29；`pnpm exec tsc -p tsconfig.json --noEmit`（在 `apps/host`）通过；用真实 `dsh` 配临时 `DSH_HOME` 做端到端验证，Host A 拉起 sidecar（pid 84173、端口 60887）退出后租约名单清空、进程保留，Host B 启动后接管到同一个 pid 和 `baseUrl`（`spawn` 零次），B 退出后该 sidecar 在宽限期内无人接手、自行退出。过程中还抓到并修掉一个真 bug：数据目录尚不存在时抢启动锁会直接 ENOENT，因为锁目录用的是非递归 `mkdir`（递归模式对已存在目录不报错，会丢掉互斥语义），现在父目录单独建。
