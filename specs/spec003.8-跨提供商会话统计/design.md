...
Codex 允许用户沿用 CLI 默认模型，创建时可能没有选中模型。对此不使用配置默认值猜价格：同一会话的统计刷新只要拿到有效的本地快照，且 `turn_context.model` 已折叠出完整目录费用，就把当前快照和绑定创建时间固定到收费字段。不能用价格快照抓取时间作为计费资格门槛，否则同一模型会因每日同步时刻出现费用显示差异。未完成 turn、并发 turn、未知模型或缺少快照仍不计费；已有会话只在显式历史同步触发的单会话刷新中按需确认，不在启动或全库扫描时隐式回填。
