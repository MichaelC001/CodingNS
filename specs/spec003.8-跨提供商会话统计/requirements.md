...
11. 目录价格必须来自版本固定的本地价格表。运行时只读本地快照，不在每轮会话统计中访问外部价格 URL；机器可读目录（当前为 `models.dev`）只能作为受控的低频同步输入。同步失败时保留最近成功快照；首次没有成功快照时不计费，也不使用内置模型价格。
...
- 不新增第二套费用计算方法、独立成本 API 或每轮网络价格查询。允许新增使用现有 `TaskManager` 的低频价格快照同步任务，以及三张只保存会话结果的 SQLite 表（统计快照、费用账单、模型用量）；费用仍必须在既有 `ProviderSessionStats` 的同一次折叠中计算。
...
17. WHEN 用户打开价格表 THEN 页面 SHALL 从当前 Host 的本地最新价格快照读取数据，并使用表格列出 GPT、Claude、GLM、Kimi、DeepSeek、Gemini 六类模型的系列、提供商、模型、输入、输出、缓存读取和缓存写入价格；价格表 SHALL 标明版本、来源和抓取时间（如有），且该操作不得直接访问 models.dev 网络。
18. 价格快照按 UTC 日期生成，同一天内容变化时追加修订版本；已绑定旧版本的会话 SHALL 继续使用原版本，不得因后续同步改变历史金额。
...
- 价格表更新不得改变已固定收费策略的会话历史金额；没有收费策略的旧 Codex 会话只能在用户显式读取触发的单会话统计刷新中，且实际模型和完整用量都可核验时，按需固定当前本地快照，不得在启动或全库扫描时自动回填。
- `models.dev` 不可访问、返回格式变化或没有匹配模型时，价格同步任务 SHALL 失败并保留旧快照；不能清空当前可用价格表。

## 6. 价格同步与持久化补充（2026-08-16）

本节扩展原有要求，替换“按周快照”和“运行时即时读取”这两个实现约束：

1. models.dev SHALL 由现有 `TaskManager` 任务每天同步一次；每次成功同步使用 UTC 日期生成不可变版本，例如 `models.dev-2026-08-16`。同一天内容发生变化时 SHALL 生成 `-r2`、`-r3` 等修订版本，禁止覆盖任何已存在的文件。
2. 本地价格目录最多保留最近 7 个 UTC 日期的快照；同一日期的修订版本全部保留。同步失败时 SHALL 保留已有快照，不得用内置模型价格补齐。
3. models.dev 的完整目录只允许存在于本地同步缓存中。会话绑定、费用账单和 runtime 响应 SHALL 只包含该会话实际出现且精确命中的模型价格条目，不得把完整目录放进费用详情或 runtime payload。
4. 新会话只有在模型名称与当前本地 models.dev 快照精确匹配、且存在明确收费策略时，才写入 `billing_started_at`、`pricing_profile_id` 和 `price_book_version`。首次离线、模型不存在、代理名称无法精确匹配、模型为空时 SHALL 不写收费元数据，也不得猜测价格。
5. Host 的 `getSessionStats()` 以及所有 runtime 读取接口 SHALL 纯读 SQLite 快照，禁止在请求内访问 Provider 日志、Harness RPC 或 models.dev 网络；Provider adapter 的 `readSessionStats()` 只能由显式后台刷新任务调用。
6. 系统 SHALL 将每条会话的统计快照、费用账单和模型用量分别保存到 SQLite；统计刷新成功后在同一事务中覆盖写入三类数据。费用不可核验时删除该会话旧账单和模型费用，不得保留过期金额。
7. 运行事件、终态事件和显式历史同步完成后 SHALL 请求 `session.stats_snapshot_refresh`，同一 session 的刷新任务必须通过 `taskType + sessionId` 去重。后台刷新失败时保留上一次成功快照，不能让 runtime 请求失败。
8. 已有会话的费用确认只能跟随显式历史同步触发的单会话刷新；启动流程和普通 runtime 请求不得自动扫描全库。
