...
- On Windows, use the npm-prebuilt N-API packages directly; no private Node.js runtime or local `node-gyp` compilation is required
...
- [libsql](https://github.com/WiseLibs/libsql) - SQLite database
...
- 在 Windows 环境下直接使用 npm 预编译 N-API 依赖，不再下载私有 Node.js 运行时或触发本机 `node-gyp` 编译
- 询问服务端口，默认 `3002`
- 询问数据目录，默认 `~/.codingns`
...
- [libsql](https://github.com/WiseLibs/libsql) - SQLite 数据库
